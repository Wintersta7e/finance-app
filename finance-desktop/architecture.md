...
- **Renderer (React)** lives under `src/`, bootstrapped by `src/main.tsx`. `src/App.tsx` renders a sidebar layout and switches between Dashboard, Accounts, Transactions, Categories, Recurring Rules, Analytics (charts), and Budgets pages that fetch backend data (accounts, categories, transactions, recurring rules, budgets, analytics, settings). UI uses shared primitives in `src/components/ui/` with tokens from `src/theme.ts`.
